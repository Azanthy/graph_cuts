...
__global__ void relabel2(int *excess, int *heights, int *tmp_heights, int *up,
        int *right, int *bottom, int *left, int width, int height, size_t pitch,
        int height_max, int *nb_active)
{
    int x = blockDim.x * blockIdx.x + threadIdx.x;
    int y = blockDim.y * blockIdx.y + threadIdx.y;
...
__global__ void any_active(int *excess, int *heights, int *nb_active, int width, int height,
        int pitch, int height_max)
{
...
void max_flow_gpu_opt(Graph graph)
{
    ...
    int *left = duplicate_on_gpu(graph._neighbors[3], width, height, pitch);
    int *nb_active;
    cudaMallocManaged(&nb_active, 1*sizeof(int));
    nb_active[0] = width * height;
    ...
    while (nb_active[0])
    {
        ...
        push<<<dimGrid, dimBlock>>>(excess, heights, up, right, bottom, left,
            width, height, pitch, size);
        // Any active kernel call
        any_active<<<1,1>>>(excess, heights, nb_active,
                width, height, pitch, size);
